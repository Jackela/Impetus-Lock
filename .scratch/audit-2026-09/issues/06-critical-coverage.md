...
- `server/api/routes/tasks.py`
  Frontend exact modules: src/services/LockManager.ts, src/components/Editor/TransactionFilter.ts, src/services/ContentInjector.ts, src/hooks/useLockEnforcement.ts, src/utils/prosemirror-helpers.ts, src/utils/textRange.ts, and any serializer helper introduced in05. EditorCore callbacks get real regression coverage in05; report overall EditorCore coverage separately rather than count unrelated toolbar/timers as lock enforcement.
- [ ] A focused regression check first fails because critical config/threshold missing; public coverage runner/config contract verifies low aggregate fails and >=80 succeeds (synthetic temporary coverage data allowed, no production mutation).
- [ ] Run all existing tests with coverage, report fixed source set, add meaningful public behavior tests for uncovered paths until line aggregate>=80 in each layer; no tautological onReject calls.
- [ ] CI main backend-tests collects all tests' coverage and applies same backend scope/threshold; CI client uses same threshold. Server split unit-only jobs must not falsely claim full-path threshold.
- [ ] Backend and frontend gates pass. Per-file uncovered areas remain visible.
