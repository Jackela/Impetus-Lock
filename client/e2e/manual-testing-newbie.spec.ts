...
test("New User Manual Journey - Complete Walkthrough", async ({ page }) => {
  ...
  const museModeText = await page
    .locator(".welcome-mode")
    .filter({ hasText: "Muse Mode" })
    .textContent();
  console.log(
    `\nMuse Mode explanation found: ${museModeText?.includes("60 seconds") ? "YES" : "NO"}`
  );

  const lokiModeText = await page
    .locator(".welcome-mode")
    .filter({ hasText: "Loki Mode" })
    .textContent();
  console.log(`Loki Mode explanation found: ${lokiModeText?.includes("Chaos") ? "YES" : "NO"}`);

  const lockConceptText = await page
    .locator(".welcome-mode")
    .filter({ hasText: "Lock Concept" })
    .textContent();
  console.log(
    `Lock Concept explanation found: ${lockConceptText?.includes("locked") ? "YES" : "NO"}`
  );
