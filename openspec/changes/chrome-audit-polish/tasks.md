...
**Validation**:
- [x] Manual test: Open app in browser, verify tab title shows "Impetus Lock - AI-Powered Writing Pressure System"
- [x] Verify title length is <70 characters (SEO best practice): ✅ 52 characters
...
**Validation**:
- [x] E2E test passes in both dev and production modes
...
**Validation**:
- [x] Component renders progress bar when `visible=true`
- [x] Component hidden when `visible=false`
- [x] Progress bar width matches `progress` prop (0-100%)
- [x] ARIA label updates with `remainingTime`
...
**Validation**:
- [x] EditorCore calls `onTimerUpdate` every second when Muse mode active
- [x] App.tsx receives updated timer state
- [x] Timer resets to 60 when user types
...
**Validation**:
- [x] Timer indicator visible in Muse mode
- [x] Timer hidden in Off/Loki modes
- [x] Progress animates from 0% to 100% over 60 seconds
...
**Validation**:
- [x] `extractLockId` returns lock ID from valid Markdown comment
- [x] `extractLockId` returns null for non-locked nodes
- [x] `findLockedRanges` returns array of all locked ranges with positions
...
**Validation**:
- [x] Decorations applied to all locked content ranges
- [x] Decorations include CSS class `'locked-content'`
- [x] Decorations include `data-lock-id` attribute
- [x] Decorations update when document changes
...
**Validation**:
- [x] Locked content has left border and background tint
- [x] Lock icon appears on hover
- [x] Styling does not break editor layout
...
**Validation**:
- [x] Badge appears next to "Muse Mode" heading
- [x] Badge styled with purple background, white text
- [x] Badge does not appear on other mode sections
...
**Validation**:
- [x] Muse Mode heading: 1.0em
- [x] Loki Mode heading: 1.0em
- [x] Lock Concept heading: 0.9em
...
**Validation**:
- [x] Footer visible at bottom of screen
- [x] Footer contains `<kbd>?</kbd>` element
- [x] Footer styled with opacity 0.5, gray color
...
- [x] Test coverage includes all 6 capabilities
...
- [x] All 6 features visible/functional in production build
...
**Validation**:
- [x] Audit test passes
- [x] Screenshot comparison shows visual improvements
- [x] Audit rating improves from 8.5/10 → ≥9.0/10
