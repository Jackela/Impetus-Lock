...
Map new SDK response/error shapes to the existing public codes/statuses: invalid key 401, quota 429, timeout 504, blocked content 400, and stopped or other upstream failures 502. Keep response-shape failures distinct: empty `candidates`, missing `content.parts`, or no text are `invalid_response` provider errors with status 502. JSON decoding or `LLMInterventionDraft` schema validation failures after text extraction must propagate as the existing Pydantic `ValidationError`; `intervention.py` catches that `ValueError` path and returns the existing `ValidationError` response with status 422. Characterize the two paths separately because the new SDK may report blocked or empty results as response metadata rather than exceptions.
